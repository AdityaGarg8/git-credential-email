...
ClientId_Thunderbird = "dj0yJmk9OXRHc1FqZHRQYzVvJmQ9WVdrOU1UQnJOR0pvTjJrbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD02NQ--"
ClientSecret_Thunderbird = "79c1c11991d148ddd02a919000d69879942fc278"

ServiceName = "git-credential-aol"
...
def AccountsUrl(command):
	return '%s/%s' % ('https://api.login.aol.com', command)
...
def RefreshToken(refresh_token):
	...
	try:
		...
		print(f"HTTP Error {response.status_code}: {response.reason}")
		print("Response Body:", response.text)
		sys.exit("\nFailed to get access token due to an HTTP error.\nTry running `git credential-aol --authenticate` to get a new refresh token.")
	except Exception as e:
		print(f"Unexpected error: {e}")
...
def main():
	if "--help" in sys.argv:
		print("""
Usage: git-credential-aol [OPTIONS]
...
Description:
  This script allows you to authenticate with AOL using OAuth2 and
...
""")
	...
	if "--set-client" in sys.argv:
		...
		print("1. Thunderbird")
		print("2. Use your own custom client details")
		...
		if need_new_access_token:
			...
			if refresh_token is None:
				sys.exit("No refresh token found.\nPlease authenticate first by running `git credential-aol --authenticate`")
